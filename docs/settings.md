...
- Truss Web Tools: bundled stdio MCP server for browser-backed web search, webpage loading, and website screenshots. On Windows, it connects only to the authenticated loopback broker owned by the global `LocalSystem` Truss service. The service lazily starts one always-headless Camoufox and stores its browser/profile data under the service-owned Truss home. MCP processes never start a local fallback. If the service or inherited capability is unavailable, browser MCP startup waits at most 15 seconds and fails with a service-required error.
- Truss Playwright Browser: bundled stdio MCP server key `truss-playwright-mcp` for Playwright MCP's interactive browser tools through the same service broker. It is written into the global service's `mcp.json` as a managed entry but disabled by default. Settings keys are `mcp.playwright_mcp_enabled` (`boolean`, default `false`) and `mcp.playwright_mcp_tools` (`string`, default `*`, comma/newline allowlist of upstream Playwright MCP tool names). Enable it, save Truss MCP Settings, and reload MCP servers to connect it.
